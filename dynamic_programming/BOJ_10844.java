...
public class Main {
  ...
  /*
      dist는 자릿수, val은 자릿값을 의미함

      첫째 자릿수는 각 val이 끝이기 때문에 경우의 수는 1밖에 없다. 
      즉, dp[1]의 각 자릿값은 1로 초기화 되어있어야 한다.
  */
  ...
  static long recur(int digit, int val) {
    ...
    // 해당 자릿수의 val 값에 대해 탐색하지 않았을 경우
    if (dp[digit][val] == null) {
      // val이 0일 경우 이전 자리는 1밖에 못옴
      if (val == 0) {
        dp[digit][val] = recur(digit - 1, 1);
      }
      // val이 1일 경우 이전 자리는 8밖에 못옴
      else if (val == 9) {
        dp[digit][val] = recur(digit - 1, 8);
      }
      // 그 외의 경우는 val - 1 과 val + 1 값의 경우의 수를 합한 경우의 수가 됨
      else {
...
class Main {
  ...
  public static int temp = 1000000000;
