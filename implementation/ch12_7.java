...
class Combination {
  ...
  public void combination(ArrayList<Position> arr, int depth, int index, int target) {
    // depth : 뽑은 개수, target : 탐색 중인 인덱스, index : now 배열의 인덱스
    if (depth == r) { // r개 뽑았으면 result에 담고 종료
    ...
    if (target == n) return; // n개 탐색 했으면 종료
    now[index] = target; // 일단 담기
    combination(arr, depth + 1, index + 1, target + 1); // target을 뽑은 경우
    combination(arr, depth, index, target + 1); // target을 뽑지 않은 경우
  }
}
