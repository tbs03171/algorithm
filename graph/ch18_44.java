  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < nodes.size() - 1; i++) {
      for (int j = i + 1; j < nodes.size(); j++) {
        int x = Math.abs(nodes.get(i).getX() - nodes.get(j).getX());
        int y = Math.abs(nodes.get(i).getY() - nodes.get(j).getY());
        int z = Math.abs(nodes.get(i).getZ() - nodes.get(j).getZ());
        int cost = Math.min(x, Math.min(y, z));
    ...
    for (int i = 0; i < edges.size(); i++) {
      ...
      if (findParent(a) != findParent(b)) {
        unionParent(a, b);
        result = result + edge.getDistance();
      }
    }
